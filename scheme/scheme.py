...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        def single_arg_eval(value):
            return scheme_eval(value, env)
        operands = rest.map(single_arg_eval)
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    new_list = expressions
    if new_list is nil:
        return None
    while new_list.second is not nil:
        scheme_eval(new_list.first, env)
        new_list = new_list.second
    return scheme_eval(new_list.first, env, True)
    # END PROBLEM 8
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("TypeError: incorrect number of arguments")
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    symbols = Pair("placeholder", nil)
    values = Pair("placeholder", nil)
    returnedsym = symbols
    returnedval = values

    while not bindings is nil:
        if not bindings.first.second.second is nil:
            raise SchemeError("incorrect number of arguments for let")
        if not scheme_symbolp(bindings.first.first):
            raise SchemeError("expected symbol found something else")
        symbols.second = Pair(bindings.first.first, nil)
        values.second = Pair(scheme_eval(bindings.first.second.first, env), nil)
        symbols = symbols.second
        values = values.second
        bindings = bindings.second

    return env.make_child_frame(returnedsym.second, returnedval.second)
